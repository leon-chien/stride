...
def split_dataset(
    ...
    """
    Random window-level split for Version 2 MVP.

    Later, when using real WESTPA trajectories, split by simulation run /
    trajectory / WESTPA iteration block to avoid leakage.
    """
...
def train_nacl(
    ...
    train_split = float(training_cfg.get("train_split", 0.7))
    ...
    print("Loading NaCl dataset...")
    print(f"Dataset path: {dataset_path}")
    print(f"Event type: {system_cfg.get('event_type')}")
    print(f"Association distance: {system_cfg.get('association_distance')}")
