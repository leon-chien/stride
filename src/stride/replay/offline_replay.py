...
def load_toy_dataset(dataset_path: str | Path) -> tuple[np.ndarray, np.ndarray]:
    """
    Load toy windows and labels from a saved .npz file.
    ...
    if not dataset_path.exists():
        raise FileNotFoundError(
            ...
            "Run `python scripts/run_toy2d.py` first."
        )
...
def load_gru_checkpoint(
    ...
    """
    Load trained GRURanker and normalization stats.
    """
...
def score_windows(
    ...
    """
    Run model inference on trajectory windows.
...
def run_offline_replay(
    dataset_path: str | Path = "outputs/toy2d/toy2d_dataset.npz",
    ...
    This asks:
        If STRIDE used the trained model to rank toy trajectory windows,
        how enriched would the selected high-value windows be for future events?
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"Using device: {device}")
    print(f"Loading dataset: {dataset_path}")
    X, y = load_toy_dataset(dataset_path)

    print(f"Loading checkpoint: {checkpoint_path}")
    model, mean, std, checkpoint = load_gru_checkpoint(checkpoint_path, device)

    X_norm = normalize_with_checkpoint_stats(X, mean, std)

    print("Scoring trajectory windows...")
