...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train STRIDE's temporal value model on WESTPA pcoord lineage windows."
    )
    ...
    split_stats = describe_pcoord_lineage_split(
        dataset=dataset,
        validation_fraction=args.validation_fraction,
        seed=args.seed,
        split_strategy=args.split_strategy,
    )
    print(f"Dataset: {args.lineage_npz}", flush=True)
    ...
    metadata = {
        "lineage_npz": str(args.lineage_npz),
        "epochs": args.epochs,
